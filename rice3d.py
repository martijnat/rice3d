...
parser = argparse.ArgumentParser(
    description='Rice3d is a command line application that draws 3d models using text and terminal escape codes.')
parser.add_argument("-b", "--blockcharacters",
                    help="Use block characters, may not work with all terminals/fonts", action="store_true")
parser.add_argument("-F", "--framerate",
                    help="Maximum framerate when drawing multiple frames", type=int, default=60)
parser.add_argument("-f", "--framecount",
                    help="Number of frames to render (ignore in combination with --singleframe)", type=int, default=-1)
parser.add_argument(
    "-c", "--columns", help="Number of columns (widht) per frame", type=int, default=columns)
parser.add_argument(
    "-l", "--lines", help="Lines of output (height) per frame", type=int, default=rows)
parser.add_argument(
    "-s", "--script", help="Output as a BASH shell script, to be run later", action="store_true")
parser.add_argument("-W", "--wireframe",
                    help="Draw model as wireframe instead of solid faces", action="store_true")
parser.add_argument("-a", "--ascpectratio",
                    help="Ratio between height and length of a character \"pixel\"", type=float, default=1.5)
parser.add_argument("-A", "--autoscale",
                    help="Automatically scale lighting to model depth", action="store_true")
parser.add_argument("-d", "--dithering",
                    help="Dither colors rather than rounding them down", action="store_true")
parser.add_argument(
    "-t", "--time", help="Time to start animation at", type=int, default=0)
parser.add_argument("-T", "--truecolor",
                    help="Use 24-bit true color output", action="store_true")
parser.add_argument("-R", "--truecolorR",
                    help="Red base value (truecolor)", type=float, default=1.0)
parser.add_argument("-G", "--truecolorG",
                    help="Green base value (truecolor)", type=float, default=1.0)
parser.add_argument("-B", "--truecolorB",
                    help="Blue base value (truecolor)", type=float, default=1.0)
parser.add_argument("-u", "--camerau",
                    help="Camera angle (u)", type=float, default=0.0)
parser.add_argument("-v", "--camerav",
                    help="Camera angle (v)", type=float, default=0.0)
parser.add_argument("-w", "--cameraw",
                    help="Camera angle (w)", type=float, default=0.0)
parser.add_argument("FILE", help=".obj file to be rendered")
parser.add_argument("-g", "--gradient",
                    help="string used to generate a character gradient", default="")
...
color_gradient = c256_gradient if TERM in [
    "screen-256color", "xterm-256color", "xterm"] else ascii_gradient

if args.blockcharacters:
    args.gradient = uc256_gradient
elif args.truecolor:
    args.gradient = ["\x1b[48;2;%i;%i;%im " % (int(i * args.truecolorR), int(
        i * args.truecolorG), int(i * args.truecolorB)) for i in range(255)]
elif args.gradient == "":
    ...
    class Scanbuffer():
        ...
        def draw_part(self, y_min, y_max):
            for y in range(max(-height, int(y_min)), min(height - 1, int(y_max))):
                draw_line(self.minX[y], y, self.maxX[y],
                          y, self.minC[y], self.maxC[y])
...
def load_obj(filename, camera):
        ...
        try:
            ...
            if c == "v":            # vertices information
                ...
                if "/" in line:  # check for a/b/c syntax
                    ...
                    indexes = list(
                        map(lambda x: (int(x) - 1), line[1:-1].split()))

                # If a face has more than 3 vertices, split the face into
                # triangles
            ...
            sys.stderr.write("Could not parse line %d: %s" %
                             (linenumber, line))
            quit(1)
    ...
    camera.zoom = zoomfactor / \
        (sum([max_x - min_x, max_y - min_y, max_z - min_z]) / 3)**2
    return faces
